...
const postStationList = async (request: { data: IRequestPostStation; params: IParamsPostStationList }) => {
  const { data, params } = request;
  return await axiosRequest<IResponsePostStation>(csmsApi, 'POST', '/ev-station/list/search', data, params);
};

export const usePostStationList = (): UseMutationResult<
  IResponsePostStation,
  AxiosError,
  { data: IRequestPostStation; params: IParamsPostStationList }
> => {
  return useMutation({
    mutationKey: ['post-station-list'],
    mutationFn: (request: { data: IRequestPostStation; params: IParamsPostStationList }) => postStationList(request),
    onSuccess: data => {
      return data;
    },
  });
